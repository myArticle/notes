...
`nutui` 是支持 `h5` 和小程序的移动端组件库，那么它是如何同时支持 `h5` 和小程序的呢？
其实 `nutui` 最初只有 `h5` 组件，后来才支持的小程序组件。所以这就是为什么，每个组件下都有一个 `index.vue` 与 `index.taro.vue` 。 
`index.vue` 对应的是 `h5` 组件，`index.taro.vue` 对应的是 `taro` 组件。引入使用的时候，也是分别从两个 `npm` 包去引入：
...
除了组件，在构建脚本中也有一部分带有 `.taro.` 的文件，以及构建命令中带有 `:taro` ，用来区分小程序组件与 `h5` 组件。
`taro` 小程序组件这部分的构建，在 [nutui源码探究——taro组件打包篇（一）](https://ishare.58corp.com/articleDetail?id=103306) 中简单介绍过。
这篇主要介绍 `npm run add` 开发一个新组件的流程。
...
- 执行 `npm run add` ，交互完成组件信息的录入
- 找到 `src/packages/__VUE/` 下对应的组件，完善组件内容
    - `index.vue`： `h5` 组件
    - `index.taro.vue`：`taro` 组件
    - `index.scss`：组件样式
    - `demo.vue`：使用示例
    - `doc.md`：使用文档
    - `__tests__/`：测试用例
- 执行 `npm run dev:taro:weapp` ，这一步就跟 `taro` 开发小程序预览一样，通过微信开发者工具打开 `src/sites/mobile-taro/vue/dist` 目录，就可以预览组件的效果。
- 测试无误后，`npm version patch` ，更新 `npm` 包版本
- `npm publish` 发布
...
<details>
<summary>jd/createComponentMode.js</summary>
